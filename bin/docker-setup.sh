...
cli()
{
	docker run -it --rm --user xfs --volumes-from woocommerce_amazon_pay_wordpress --network container:woocommerce_amazon_pay_wordpress wordpress:cli "$@" > /dev/null
}
...
cli wp core install --path=/var/www/html --url=localhost:8082 --title="WooCommerce Amazon Pay Dev" --admin_name=admin --admin_password=admin --admin_email=admin@example.com --skip-email
...
echo "Activating the WooCommerce Amazon Payments Gateway Advanced plugin..."
cli wp plugin activate woocommerce-gateway-amazon-payments-advanced
