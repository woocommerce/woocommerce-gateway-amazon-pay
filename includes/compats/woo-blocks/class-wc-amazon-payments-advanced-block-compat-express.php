...
class WC_Amazon_Payments_Advanced_Block_Compat_Express extends WC_Amazon_Payments_Advanced_Block_Compat_Abstract {
	...
	public function get_payment_method_data() {
		...
		return array_merge(
			...
			array(
				...
				'amazonAddress'         => array(
					'amazonBilling'  => $checkout_session && ! is_wp_error( $checkout_session ) && ! empty( $checkout_session->billingAddress ) ? WC_Amazon_Payments_Advanced_API::format_address( $checkout_session->billingAddress ) : null,
					'amazonShipping' => $checkout_session && ! is_wp_error( $checkout_session ) && ! empty( $checkout_session->shippingAddress ) ? WC_Amazon_Payments_Advanced_API::format_address( $checkout_session->shippingAddress ) : null,
				),
			)
		);
	}
	...
	protected function scripts_name_per_type( $type = '' ) {
		...
		$script_data = include wc_apa()->path . '/build/payments-methods/express/index' . $min . '.asset.php';
		wp_register_script( 'amazon_payments_advanced_express_block_compat', wc_apa()->plugin_url . '/build/payments-methods/express/index' . $min . '.js', $script_data['dependencies'], $script_data['version'], true );

		/* Registering Regular Payment Script, which takes over after user is logged in via Amazon. */
		$script_helper_data = include wc_apa()->path . '/build/payments-methods/express-helper/index' . $min . '.asset.php';
		wp_register_script( 'amazon_payments_advanced_express-helper_block_compat', wc_apa()->plugin_url . '/build/payments-methods/express-helper/index' . $min . '.js', $script_helper_data['dependencies'], $script_helper_data['version'], true );
